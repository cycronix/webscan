...
//	var rtime = new Date().getTime();		// request time
...
//				console.debug("audio fetch time: "+(new Date().getTime() - rtime)+', pts: '+audioRequest.response.byteLength+', url: '+url);
...
//						if(debug) 
							console.debug('New PLAYDELAY: '+playDelay);
					}
					else {
...
//					playDelay = new Date().getTime() - newTime[param];
					playDelay = 10*fastDelay + new Date().getTime() - newestTime;
		...
		if (xmlhttp.readyState==4) {
			...
			if(xmlhttp.status==200) {
				...
				if(ptime > newestTime) 
					newestTime = ptime;
				if(debug) 
					console.debug("AjaxGetParamTimeNewest, param: "+param+", time: "+newTime[param]);
			}
			else {  				
